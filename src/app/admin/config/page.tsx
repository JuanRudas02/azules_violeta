...
export default function AdminConfigPage() {
    ...
    const [pointsAwarded, setPointsAwarded] = useState(10); // Default $10k = 10pts
    const [adminEmail, setAdminEmail] = useState('soporte@azulesvioleta.com');
    const [adminPhone, setAdminPhone] = useState('+57 300 000 0000');
    ...
    const fetchConfigs = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const divisorConfig = configs.find((c: any) => c.key === 'POINTS_PER_PURCHASE_DIVISOR');
                if (divisorConfig) {
                    // divisor = 10000 / pointsAwarded  => pointsAwarded = 10000 / divisor
                    const pts = 10000 / parseInt(divisorConfig.value);
                    setPointsAwarded(pts);
                }
                const emailConfig = configs.find((c: any) => c.key === 'ADMIN_SUPPORT_EMAIL');
                if (emailConfig) setAdminEmail(emailConfig.value);

                const phoneConfig = configs.find((c: any) => c.key === 'ADMIN_SUPPORT_PHONE');
                if (phoneConfig) setAdminPhone(phoneConfig.value);
            }
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            // Save Points Divisor
            const divisor = 10000 / pointsAwarded;
            await fetch('/api/admin/config', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'POINTS_PER_PURCHASE_DIVISOR', value: divisor })
            });
            ...
            await fetch('/api/admin/config', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'ADMIN_SUPPORT_EMAIL', value: adminEmail })
            });
            ...
            await fetch('/api/admin/config', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ key: 'ADMIN_SUPPORT_PHONE', value: adminPhone })
            });
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="md:col-span-1 space-y-2">
                        {[
                            ...
                            { label: 'Reglas de Puntos', icon: Coins, active: true },
                    ...
                    <div className="md:col-span-2 space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="bg-secondary/30 p-4 rounded-2xl">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-transparent font-black text-primary text-xl outline-none"
                                    />
                                </div>
                            </div>
                            <p className="text-xs text-gray-400 italic font-medium">
                                * Por cada $10.000 COP que la cliente registre, el sistema le asignará automáticamente {pointsAwarded} puntos.
                            </p>
                        </motion.section>
                        ...
                        >
                            ...
                            <p className="text-xs text-rose-400">Acciones críticas para la administración de la marca.</p>
